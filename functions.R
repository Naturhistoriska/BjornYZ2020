...
prov_table_lan1 <- function(data){
  veckovis <- data %>% 
    select(Individ, Kon, Vecka, LnNamn) %>% 
    distinct() %>% 
    group_by(Individ, Kon, LnNamn) %>% 
    summarise(n = n()) %>% 
    ungroup() %>% 
    group_by(Kon, LnNamn) %>% 
    summarise(antal = paste0(sum(n), " (", round(mean(n), 2), ")"))
  veckovis_hela <- data %>% 
    select(Individ, Kon, Vecka) %>% 
    distinct() %>% 
    group_by(Individ, Kon) %>% 
    ...
    group_by(Kon) %>% 
    summarise(antal = paste0(sum(n), " (", round(mean(n), 2), ")")) %>% 
    mutate(LnNamn = "Totalt")
  bind_rows(veckovis, veckovis_hela) %>% 
    pivot_wider(names_from = "Kon", values_from = "antal")
}

prov_table_lan2 <- function(data){
  total <- data %>% 
    select(Individ, Kon, LnNamn) %>% 
    group_by(Individ, Kon, LnNamn) %>% 
    summarise(n = n()) %>% 
    ungroup() %>% 
    group_by(Kon, LnNamn) %>% 
